...
# modify by junlysky.info@gmail.com
sub plot {
  ...
  ($nn,$hight) = (15,25.5);	# 10 rows of traces per 10.5 in.
  
  $sepa = 0.1*$sec_per_inch;
  ($tt, $inc) = (2*$t1 + 3*$t2 + 4*$sepa, 1);
  ($tt, $inc) = ($t1 + $t2 + $sepa, 4) if $num_com == 2;
  $width = 0.2*int(12*$tt/$sec_per_inch+0.5);
  ...
  $plt1 = "| pssac2 -JX$width/$hight -R0/$tt/0/$nn -Y3 -Ent-2 -M$am -P -K >> $outps";
  $plt2 = "| pstext -JX$width/$hight -R0/$tt/0/$nn -N -K -O >> $outps";
  $plt3 = "| psmeca -JX1/1 -R-1/1/-1/1 -Sm15  -Y24 -X-1 -N -K -O  >> $outps" if $tensor[1] eq "MomentTensor";
  $plt4 = "| psxy -JPa1 -R0/360/0/1 -Sx0.1 -W2/255/0/0 -N -O >> $outps";
